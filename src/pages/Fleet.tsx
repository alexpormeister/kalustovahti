...
export default function Fleet() {
  ...
  const { data: vehicles = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch attributes for each vehicle
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      ...
      if (data.selected_attributes.length > 0) {
        const links = data.selected_attributes.map((attrId) => ({
          vehicle_id: newVehicle.id,
          attribute_id: attrId,
        }));
        await supabase.from("vehicle_attribute_links").insert(links);
      }
    },
    onSuccess: () => {
  ...
  const filteredVehicles = vehicles.filter((vehicle) => {
    ...
    const matchesAttributes =
      ...
      attributeFilters.every((filterId) =>
        vehicle.attributes?.some((attr) => attr.id === filterId)
      );
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <Input
            ...
            onChange={(e) =>
              setFormData({ ...formData, payment_terminal_id: e.target.value })
            }
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) =>
              setFormData({ ...formData, meter_serial_number: e.target.value })
            }
                  ...
                  onCheckedChange={(checked) => {
                    setFormData((prev) => ({
                      ...
                      selected_attributes: checked
                        ...
                        : prev.selected_attributes.filter(
                            (id) => id !== attr.id
                          ),
                    }));
                  }}
              ...
              <div className="flex flex-wrap gap-2">
                {attributes.map((attr) => (
                  <Badge
                    key={attr.id}
                    variant={
                      attributeFilters.includes(attr.id) ? "default" : "outline"
                    }
